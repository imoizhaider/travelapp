    ...
    <Card className="h-full">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-3 text-base font-semibold text-white">{hotel.hotel_name}</h3>
          <p className="mt-1 text-sm text-slate-400">{hotel.hotel_description || 'Hotel listing for itinerary planning.'}</p>
        </div>
        <Badge>{hotel.star_rating || '—'}★</Badge>
      </div>
      <div className="mt-4 rounded-xl border border-white/6 bg-white/3 p-4 text-sm text-slate-400">
