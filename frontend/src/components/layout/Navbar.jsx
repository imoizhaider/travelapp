...
const navLinkClass = ({ isActive }) => `rounded-xl px-3 py-2 text-sm font-medium transition ${isActive ? 'bg-white/10 text-white' : 'text-slate-400 hover:bg-white/8 hover:text-white'}`;
...
export default function Navbar({ onMenuClick }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/8 bg-surface-950/70 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <Link to="/" className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-accent-blue to-accent-cyan text-white shadow-lift">
              <Compass className="h-5 w-5" />
            </div>
            <div>
              <div className="font-display text-lg text-white">Travel Planner</div>
              <div className="text-xs text-slate-500">Plan with precision</div>
            </div>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <>
              <div className="hidden items-center gap-3 rounded-xl border border-white/8 bg-white/4 px-3 py-2 md:flex">
