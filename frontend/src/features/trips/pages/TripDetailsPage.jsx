...
export default function TripDetailsPage() {
  ...
  if (tripState.loading) {
    return (
      <div className="space-y-8 animate-fadeUp">
        <Skeleton variant="card" count={1} className="min-h-[200px]" />
        <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
          <Skeleton variant="card" count={1} className="min-h-[300px]" />
  ...
  if (!trip) {
    if (tripState.error) {
      return <div className="space-y-4 animate-fadeUp"><Alert title="Trip details unavailable" message={tripState.error} /><EmptyState title="Trip unavailable" description="The trip could not be loaded from the API." /></div>;
    }
