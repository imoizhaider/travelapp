  ...
  if (destinations.loading) {
    return (
      <div className="mx-auto max-w-3xl space-y-6 animate-fadeUp">
        <div>
          <h1 className="text-2xl font-semibold text-slate-800">Create Trip</h1>
          <p className="mt-1 text-sm text-slate-500">Build a structured trip with destination, schedule, and traveler details.</p>
        </div>
      ...
      <Card>
        <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
          ...
          <label className="block space-y-2">
            ...
            <select value={form.tripStatus} onChange={(event) => setForm({ ...form, tripStatus: event.target.value })} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-700 outline-none">
